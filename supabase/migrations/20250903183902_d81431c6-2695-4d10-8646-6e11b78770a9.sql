-- Aplicar sistema de prevenção de duplicatas (versão final corrigida)

-- 1. Criar índice único parcial apenas para laboratórios
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_laboratory_per_date 
ON public.reservations (equipment_type, reservation_date) 
...
BEGIN
  ...
  SELECT r.id, r.reservation_date, r.created_at, p.display_name 
  INTO reservation_id, reservation_date, reservation_created_at, user_display_name
  FROM reservations r
