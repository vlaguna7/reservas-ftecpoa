-- Drop and recreate verify_user_login with proper security
DROP FUNCTION public.verify_user_login(text, text);
...
BEGIN
  ...
  -- Return only approved users and no sensitive data
  RETURN QUERY
  SELECT 
    p.user_id,
